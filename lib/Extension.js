    ...
    module.exports = new Class({
        ...
        , onBeforePrepare: function(resource, definition) {
            ...
            resource.getQuery().select.some(function(selection) {
                if (this._database[resource.databaseName] && this._database[resource.databaseName][resource.name]) {
                    if (selection === '*') {
                        // we need to select the language table, all fieds
                        Object.keys(this._database[resource.databaseName][resource.name]).forEach(function(columnName) {
                    ...
                    else if (this._database[resource.databaseName][resource.name][selection]) {
        ...
        , _addSubSelect: function(fieldName, resource) {
            var   definition        = this._database[resource.databaseName][resource.name][fieldName]
            ...
            if (!resource._localizedSelection || !resource._localizedSelection[fieldName]) {
        ...
        , setQueryLanguageSelection: function(languageSelection) {
            this.getResource().languageSelection = languageSelection;
            return this;
        }
        ...
        , applyModelMethods: function(definition, classDefinition) {
            ...
            Object.keys(this._database[definition.databaseName][definition.name]).forEach(function(columnName) {
        ...
        , useOnModel: function(definition) {
            ...
            if (Object.keys(definition.columns).some(function(columnName) {
                ...
                this._database[definition.databaseName] = {};
                this._database[definition.databaseName][definition.name] = {};
                ...
                Object.keys(mappingDefinition.via.model.columns).forEach(function(col) {
                    ...
                    if (!localColumn.isPrimary) {
                        this._database[definition.databaseName][definition.name][localColumn.name] = config;
                    }
                }.bind(this));              
