!function() {
    ...
    module.exports = new Class({
        ...
        , onBeforeSave: function(model, transaction, callback) {
            ...
            if (!model.getDefinition().isLocalizedTable) callback();


            //is there any data set
            if (!model._changedValues.length) callback();


            // check if the mode._serialize object contains properties
            // that are managed by us
            model._changedValues.forEach(function(propertyName) {
                var id, config;
                ...
                if (this._storage[databaseName][tableName][propertyName]) {
                    ...
                    // did we load the records already?
                    ...
                    records[databaseName][config.mappingName].values[propertyName] = model[propertyName];

                    changedValues.push(propertyName);
                }
            }.bind(this));

            
            // safe data on the model, we will persist after 
            // the model was saved
            model._languageRecords = records;

            // remove from model
            changedValues.forEach(function(name) {
                model._changedValues.splice(model._changedValues.indexOf(name), 1);
            }.bind(this));
            ...
            Promise.all(Object.keys(records).map(function(dbName) {
                return Promise.all(Object.keys(records[dbName]).map(function(table) {
                    ...
                    mappingFilter[config.referencingColumn] = model[config.referencedColum];

                    // get record
                    return this.orm[dbName][table](mappingFilter).get(this.languageTable, languageFilter).findOne().then(function(mappingRecord) {
        ...
        , onBeforePrepareSubqueries: function(resource, definition) {
            ...
            selected.forEach(function(select, index) {
                ...
                if (this._storage[resource.databaseName][resource.name][select]) {
